...
void bindPyColorSpace(py::module & m)
{
    ...
    auto cls = py::class_<ColorSpace, ColorSpaceRcPtr /* holder */>(m, "ColorSpace", DS(ColorSpace))
        .def(py::init([]() { return ColorSpace::Create(); }), DS(ColorSpace, ColorSpace))
        .def(py::init([](ReferenceSpaceType referenceSpace) 
            { 
                return ColorSpace::Create(referenceSpace); 
            }),
             DS(ColorSpace, ColorSpace, 2), 
             "referenceSpace"_a)
        .def(py::init([](ReferenceSpaceType referenceSpace,
        ...
        // Transform
        .def("getTransform", &ColorSpace::getTransform, DS(ColorSpace, getTransform), "dir"_a)
        .def("setTransform", &ColorSpace::setTransform, DS(ColorSpace, setTransform), "transform"_a, "dir"_a);
