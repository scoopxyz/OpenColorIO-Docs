...
void bindPyProcessor(py::module & m)
{
    auto cls = py::class_<Processor, ProcessorRcPtr /* holder */>(m, "Processor", DS(Processor))
        ...
        .def("write", [](ProcessorRcPtr & self, const std::string & formatName) 
            {
             ...
             DS(Processor, write),
             "formatName"_a)
        .def("getDynamicProperty", &Processor::getDynamicProperty,
             DS(Processor, getDynamicProperty),
             "type"_a)
        .def("hasDynamicProperty", &Processor::hasDynamicProperty,
             DS(Processor, hasDynamicProperty),
             "type"_a)
        .def("getOptimizedProcessor",
             (ConstProcessorRcPtr(Processor::*)(OptimizationFlags) const)
             &Processor::getOptimizedProcessor,
             DS(Processor, getOptimizedProcessor),
             "oFlags"_a)
        .def("getOptimizedProcessor",
        ...
        .def("getDefaultGPUProcessor", &Processor::getDefaultGPUProcessor,
             DS(Processor, getDefaultGPUProcessor))
        .def("getOptimizedGPUProcessor", &Processor::getOptimizedGPUProcessor,
             DS(Processor, getOptimizedGPUProcessor),
             "oFlags"_a)

        // CPU Renderer
        .def("getDefaultCPUProcessor", &Processor::getDefaultCPUProcessor,
             DS(Processor, getDefaultCPUProcessor))
        .def("getOptimizedCPUProcessor", 
             (ConstCPUProcessorRcPtr (Processor::*)(OptimizationFlags) const) 
             &Processor::getOptimizedCPUProcessor,
             DS(Processor, getOptimizedCPUProcessor),
             "oFlags"_a)
        .def("getOptimizedCPUProcessor", 
