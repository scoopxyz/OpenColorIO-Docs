 ...
 *
 * \note All functions in the Config class can potentially throw this exception.
*/
...
/**
 * \brief An exception class for errors detected at
 * runtime, thrown when OCIO cannot find a file that is expected to
 * exist.
 * 
 * This is provided as a custom type to distinguish cases where one wants to continue looking for
...
/// \note The logging mechanism is thread-safe.
 ...
 *
 * The color configuration \ref Config is the main object for
 * interacting with this library. It encapsulates all of the information
 * necessary to use customized \ref `ColorSpaceTransform` and
 * \ref `DisplayTransform` operations.
 *
...
public:
    ...
    /// variable is missing or empty, returns the same result as \ref Config::CreateRaw`.
    ...
    /// If not empty or null a single character to separate
    ...
    /// \note
    ///    Setting the \ref colorSpaceName name to a null string unsets it.
    ...
    /// Remove the view and the display if no more views. It does not remove
    ...
    /// \note The setter does not override the envvar.  The getter does not take into
    /// account the envvar value and thus may not represent what the user is seeing.
     ...
     *   these functions on the \ref `Config` class. However, it's
    ...
    /// filePath.  The \ref FileRules::getNumCustomKeys and custom keys methods
    ...
    // Create a \ref `Processor` to assemble a transformation between two
    // color spaces.  It may then be used to create a \ref `CPUProcessor`
    // or \ref `GPUProcessor` to process/convert pixels.
 ...
 *   See \ref FileRules::setDefaultRuleColorSpace`.
 *   It has the keys:
 *
...
public:
    ...
    /// \note Will do nothing if the category already exists.
    void addCategory(const char * category);
    /// Remove a category.
    /// \note Will do nothing if the category is missing.
    void removeCategory(const char * category);
    /// Get the number of categories.
    int getNumCategories() const;
    /// Return the category name using its index
    /// \note Will be null if the index is invalid.
     ...
     * \ref `DisplayTransform` process obeys special 'data min' and
     * 'data max' args.
     *
...
public:
    ...
    /// \note The comparison is done on the color space names (not a deep comparison).
...
/**
 * \brief A *ViewTransform* provides a conversion from the main (usually scene-referred) reference space
...
public:
    ...
    /// See \ref ColorSpace::getFamily
    const char * getFamily() const noexcept;
    /// See \ref ColorSpace::setFamily
    void setFamily(const char * family);

    const char * getDescription() const noexcept;
    void setDescription(const char * description);

    /// See \ref ColorSpace::hasCategory.
    bool hasCategory(const char * category) const;
    /// See \ref ColorSpace::addCategory.
    void addCategory(const char * category);
    /// See \ref ColorSpace::removeCategory.
    void removeCategory(const char * category);
    /// See \ref ColorSpace::getNumCategories.
    int getNumCategories() const;
    /// See \ref ColorSpace::getCategory.
    const char * getCategory(int index) const;
    /// See \ref ColorSpace::clearCategories.
    void clearCategories();
...
public:
    ...
    /// Run the optimizer on a Processor to create a new \ref Processor.
    ...
    /// Create a \ref Processor that is optimized for a specific in and out
...
public:
     ...
     *   in the \ref Processor it was generated from.
     */
    DynamicPropertyRcPtr getDynamicProperty(DynamicPropertyType type) const;
...
public:
    ...
    /// \param inputSpace The input ColorSpace to use.
...
public:
    ...
    /// \note
    ///   that although PlanarImageDesc is powerful enough to also describe
    ///   all \ref PackedImageDesc scenarios, it is recommended to use
